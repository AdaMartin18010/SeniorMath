...
### 例题1：导数的定义 | Example 1: Definition of Derivative

**题目 | Problem**：求函数f(x) = x²在x=1处的导数。
**解答 | Solution**：
f'(x) = 2x，f'(1) = 2×1 = 2。
The derivative at x=1 is 2.

### 例题2：常见函数的导数 | Example 2: Derivative of Common Functions

**题目 | Problem**：已知y = eˣsinx，求y'。
**解答 | Solution**：
y' = eˣsinx + eˣcosx。
The derivative is y' = eˣsinx + eˣcosx.

### 例题3：切线方程 | Example 3: Equation of Tangent Line

**题目 | Problem**：已知y = x³ - 3x² + 2x，求y在x=1处的切线方程。
**解答 | Solution**：
...
### 例题4：高阶导数 | Example 4: Higher Order Derivative

**题目 | Problem**：已知f(x) = ln(x² + 1)，求f''(x)。
**解答 | Solution**：
f'(x) = 2x/(x²+1)
f''(x) = [2(x²+1) - 2x×2x]/(x²+1)² = (2-2x²)/(x²+1)²
The second derivative is f''(x) = (2-2x²)/(x²+1)².

### 例题5：单调性与极值 | Example 5: Monotonicity and Extremum

**题目 | Problem**：已知f(x) = x³ - 3x² + 2x，求其极值点。
**解答 | Solution**：
f'(x) = 3x² - 6x + 2
令f'(x)=0，解得x=[6±√(36-24)]/6=[6±2√3]/6=1±√3/3
极值点x₁=1+√3/3，x₂=1-√3/3
The extremum points are x₁=1+√3/3, x₂=1-√3/3.

## 创新与挑战性例题 | Innovative and Challenging Examples

### 例题6：隐函数求导 | Example 6: Implicit Differentiation

**题目 | Problem**：已知x²+y²=1，求y关于x的导数。
**解答 | Solution**：
对两边求导：2x+2y·y'=0，y'=-x/y。
By implicit differentiation, y'=-x/y.

### 例题7：参数方程求导 | Example 7: Derivative of Parametric Equations

**题目 | Problem**：已知x=cosθ，y=sinθ，求dy/dx。
**解答 | Solution**：
dy/dθ=cosθ，dx/dθ=−sinθ，dy/dx=cosθ/(−sinθ)=−cotθ。
The derivative dy/dx = -cotθ.

### 例题8：高阶导数与递推 | Example 8: Higher Order Derivative and Recursion

**题目 | Problem**：已知f(x)=e^{2x}sinx，求f''(x)。
**解答 | Solution**：
f'(x)=2e^{2x}sinx+e^{2x}cosx=e^{2x}(2sinx+cosx)
f''(x)=e^{2x}(4sinx+4cosx−sinx)=e^{2x}(3sinx+4cosx)
The second derivative is f''(x)=e^{2x}(3sinx+4cosx).

---

> 以上例题覆盖导数与微分核心知识点，配有详细解答，便于学习者理解与创新训练。
> The above examples cover the core knowledge points of derivatives and differentials, with detailed solutions to help learners understand and practice.
