...
**题目**：求极限 limₓ→0 (sinx)/x。
**解答**：
limₓ→0 (sinx)/x = 1。

### 例题2：极限的存在性

**题目**：判断函数f(x) = 1/x 在x=0处是否有极限。
**解答**：
左极限为-∞，右极限为+∞，极限不存在。

### 例题3：无穷小与无穷大

**题目**：判断x→0时，x²与x哪个是无穷小量？
**解答**：
x²比x更“高阶”的无穷小。

### 例题4：连续性的判定

**题目**：判断函数f(x) = {x², x≠1; 2, x=1} 在x=1处是否连续。
**解答**：
limₓ→1 f(x) = 1，f(1) = 2，不连续。

### 例题5：极限的运算

**题目**：求极限 limₓ→1 (x²-1)/(x-1)。
**解答**：
...
**题目**：求极限 limₓ→0 x²sin(1/x)。
**解答**：
|sin(1/x)|≤1，|x²sin(1/x)|≤x²，x→0时x²→0，故极限为0。

### 例题8：无穷小比较

**题目**：判断x→0时，ln(1+x)与x哪个是高阶无穷小？
**解答**：
ln(1+x)≈x−x²/2，x²/2为更高阶无穷小，故ln(1+x)与x同阶。

---

> 以上创新与挑战性例题进一步提升了极限与连续模块的深度与广度，便于学习者进阶训练。
