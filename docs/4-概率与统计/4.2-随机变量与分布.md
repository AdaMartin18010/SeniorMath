...
### 例题1：离散型随机变量 | Example 1: Discrete Random Variable

**题目 | Problem**：抛掷两枚骰子，X表示点数和，求X的所有可能取值及其概率分布。
**解答 | Solution**：
X取值2~12，各概率分别为：
...
### 例题2：概率分布与期望 | Example 2: Probability Distribution and Expectation

**题目 | Problem**：某随机变量X的概率分布为P(X=1)=0.3, P(X=2)=0.7，求E(X)。
**解答 | Solution**：
E(X)=1×0.3+2×0.7=1.7。
The expectation is 1.7.

### 例题3：二项分布 | Example 3: Binomial Distribution

**题目 | Problem**：抛掷3枚硬币，求恰有2次正面的概率。
**解答 | Solution**：
C(3,2)×(1/2)²×(1/2)=3×1/4×1/2=3/8。
The probability is 3/8.

### 例题4：连续型随机变量 | Example 4: Continuous Random Variable

**题目 | Problem**：设X在[0,1]上均匀分布，求P(X>0.6)。
**解答 | Solution**：
P(X>0.6)=1-0.6=0.4。
The probability is 0.4.

### 例题5：方差计算 | Example 5: Variance Calculation

**题目 | Problem**：某随机变量X的概率分布为P(X=1)=0.5, P(X=3)=0.5，求方差D(X)。
**解答 | Solution**：
E(X)=1×0.5+3×0.5=2
...
### 例题6：分布函数与概率计算 | Example 6: Distribution Function and Probability Calculation

**题目 | Problem**：设X在[0,2]上均匀分布，求P(0.5<X<1.5)。
**解答 | Solution**：
P(0.5<X<1.5)=(1.5-0.5)/(2-0)=1/2
The probability is 1/2.

### 例题7：二项分布应用 | Example 7: Application of Binomial Distribution

**题目 | Problem**：某产品合格率为0.9，独立抽取5件，求恰有4件合格的概率。
**解答 | Solution**：
P=C₅⁴×0.9⁴×0.1=5×0.6561×0.1=0.32805
The probability is 0.32805.

### 例题8：期望与方差综合 | Example 8: Expectation and Variance

**题目 | Problem**：设X的概率分布为P(X=0)=0.2, P(X=1)=0.5, P(X=2)=0.3，求E(X²)。
**解答 | Solution**：
E(X²)=0²×0.2+1²×0.5+2²×0.3=0+0.5+1.2=1.7
The expectation of X² is 1.7.

---

> 以上例题覆盖随机变量与分布核心知识点，配有详细解答，便于学习者理解与创新训练。
> The above examples cover the core knowledge points of random variables and distributions, with detailed solutions to help learners understand and practice.
