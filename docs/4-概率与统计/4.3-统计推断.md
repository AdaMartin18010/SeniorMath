...
### 例题1：参数估计 | Example 1: Parameter Estimation

**题目 | Problem**：某工厂生产的零件长度服从正态分布，已知标准差σ=2，抽取样本5个，测得均值为50。请估计总体均值μ的置信区间（置信度95%）。
**解答 | Solution**：
查标准正态分布表，Z₀.₀₂₅=1.96。
置信区间为：50±1.96×2/√5 ≈ 50±1.75，即(48.25, 51.75)。
The 95% confidence interval for μ is (48.25, 51.75).

### 例题2：假设检验 | Example 2: Hypothesis Testing

**题目 | Problem**：某药品声称治愈率为80%，现随机抽取100人，治愈80人。检验该药品的治愈率是否为80%（显著性水平0.05）。
**解答 | Solution**：
原假设H₀：p=0.8，样本比例p̂=0.8。
Z=|(0.8-0.8)|/√[0.8×0.2/100]=0。
查表Z₀.₀₂₅=1.96，0<1.96，故不拒绝原假设。
There is no evidence to reject the claim that the cure rate is 80% at the 0.05 significance level.

### 例题3：回归分析 | Example 3: Regression Analysis

**题目 | Problem**：已知一组数据(x,y)：(1,2),(2,3),(3,5)，求y关于x的最小二乘回归直线。
**解答 | Solution**：
设回归方程y=a+bx。
计算b=[(1-2)(2-3)+(2-2)(3-3)+(3-2)(5-3)]/[(1-2)²+(2-2)²+(3-2)²]=[(−1)(−1)+0×0+1×2]/[1+0+1]=1+2/2=1.5。
a=ȳ−b×x̄=3.33−1.5×2=0.33。
回归方程为y=0.33+1.5x。
The least squares regression line is y=0.33+1.5x.

### 例题4：方差分析 | Example 4: Analysis of Variance (ANOVA)

**题目 | Problem**：有三组数据，A组均值5，B组均值6，C组均值7，组内方差均为1，每组各5个样本。检验三组均值是否有显著差异（α=0.05）。
**解答 | Solution**：
总均值=6。
组间平方和SSB=5×[(5−6)²+(6−6)²+(7−6)²]=5×[1+0+1]=10。
组内平方和SSW=3×4×1=12。
F=（SSB/2）/（SSW/12）=5/1=5。
查表F₀.₀₅(2,12)=3.89，5>3.89，拒绝原假设。
There is a significant difference among the means of the three groups.

## 创新与挑战性例题 | Innovative and Challenging Examples

### 例题5：双样本均值检验 | Example 5: Two-Sample Mean Test

**题目 | Problem**：A、B两种教学法分别对10名学生测试，A组均值80，标准差5，B组均值85，标准差4。问两组均值差异是否显著（α=0.05）。
**解答 | Solution**：
T=|(80−85)|/√[(5²/10)+(4²/10)]=5/√(2.5+1.6)=5/√4.1≈2.47。
查t₀.₀₂₅(18)=2.101，2.47>2.101，差异显著。
The difference between the two means is significant at the 0.05 level.

### 例题6：卡方检验 | Example 6: Chi-Square Test

**题目 | Problem**：某调查中，男生喜欢数学的有40人，不喜欢的有10人，女生喜欢的有30人，不喜欢的有20人。问性别与喜欢数学是否有关（α=0.05）。
**解答 | Solution**：
构建列联表，计算期望频数，卡方统计量χ²=Σ(实际-期望)²/期望≈4.76。
查表χ²₀.₀₅(1)=3.84，4.76>3.84，拒绝独立性假设。
There is a significant association between gender and preference for mathematics.

### 例题7：置信区间宽度分析 | Example 7: Confidence Interval Width Analysis

**题目 | Problem**：样本容量n=25，均值100，标准差10，置信度99%。求μ的置信区间宽度。
**解答 | Solution**：
Z₀.₀₀₅=2.58，区间宽度=2×2.58×10/√25=2×2.58×2=10.32。
The width of the 99% confidence interval is 10.32.

---

> 以上例题涵盖统计推断的参数估计、假设检验、回归分析、方差分析等核心内容，均配有中英双语详细解答，便于学习者理解与创新训练。
> The above examples cover core topics of statistical inference, including parameter estimation, hypothesis testing, regression analysis, and ANOVA, with detailed bilingual solutions to facilitate understanding and creative practice.
