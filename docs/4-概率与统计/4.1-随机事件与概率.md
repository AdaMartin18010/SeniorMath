...
### 例题1：基本概率计算 | Example 1: Basic Probability Calculation

**题目 | Problem**：抛掷一枚硬币，正面朝上的概率是多少？
**解答 | Solution**：
正面概率=1/2。
The probability of heads is 1/2.

### 例题2：等可能事件概率 | Example 2: Probability of Equally Likely Events

**题目 | Problem**：从1~10中随机取一个数，取到偶数的概率是多少？
**解答 | Solution**：
偶数有2,4,6,8,10，共5个，概率=5/10=1/2。
There are 5 even numbers, so the probability is 1/2.

### 例题3：互斥事件概率 | Example 3: Probability of Mutually Exclusive Events

**题目 | Problem**：从1~5中随机取一个数，取到1或3的概率是多少？
**解答 | Solution**：
P(1或3)=P(1)+P(3)=1/5+1/5=2/5。
The probability is 2/5.

### 例题4：独立事件概率 | Example 4: Probability of Independent Events

**题目 | Problem**：抛掷两枚硬币，均为正面的概率是多少？
**解答 | Solution**：
P(正,正)=1/2×1/2=1/4。
The probability is 1/4.

### 例题5：条件概率 | Example 5: Conditional Probability

**题目 | Problem**：袋中有3个红球2个白球，随机取出一个是红球的概率是多少？若已知取出的是彩球，求是红球的条件概率。
**解答 | Solution**：
P(红)=3/5。
若已知取出的是彩球（假设彩球=红球+白球），P(红|彩)=3/5。
The probability of red is 3/5. If a colored ball is taken (red or white), the conditional probability is also 3/5.

## 创新与挑战性例题 | Innovative and Challenging Examples

### 例题6：多步事件概率 | Example 6: Probability of Multi-step Events

**题目 | Problem**：袋中有3个红球2个白球，连续取两次，每次取后放回，求两次都取到红球的概率。
**解答 | Solution**：
每次取红球概率3/5，两次都取到红球概率=(3/5)×(3/5)=9/25。
Each time the probability is 3/5, so the probability for both is 9/25.

### 例题7：条件概率综合 | Example 7: Comprehensive Conditional Probability

**题目 | Problem**：已知A、B为独立事件，P(A)=0.4，P(B)=0.5，求P(A∪B)。
**解答 | Solution**：
P(A∪B)=P(A)+P(B)−P(A)P(B)=0.4+0.5−0.2=0.7
The probability is 0.7.

### 例题8：概率逆向思维 | Example 8: Reverse Probability Thinking

**题目 | Problem**：抛掷两枚骰子，已知点数和为7，求其中一枚为4的概率。
**解答 | Solution**：
点数和为7的组合有(1,6),(2,5),(3,4),(4,3),(5,2),(6,1)，共6种，其中有4的为(3,4),(4,3)，共2种，概率=2/6=1/3。
There are 6 combinations with sum 7, 2 of which have a 4, so the probability is 1/3.

---

> 以上例题覆盖随机事件与概率核心知识点，配有详细解答，便于学习者理解与创新训练。
> The above examples cover the core knowledge points of random events and probability, with detailed solutions to help learners understand and practice.
