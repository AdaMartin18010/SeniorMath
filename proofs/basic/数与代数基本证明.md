...
- [数与代数基本证明 | Basic Proofs in Numbers and Algebra](#数与代数基本证明--basic-proofs-in-numbers-and-algebra)
  ...
  - [2.1 整数运算 | Integer Operations](#21-整数运算--integer-operations)
    - [典型证明1 | Example Proof 1](#典型证明1--example-proof-1-1)
    - [典型证明2 | Example Proof 2](#典型证明2--example-proof-2-1)
    - [典型证明3 | Example Proof 3](#典型证明3--example-proof-3-1)
  - [1.2 数轴与区间 | Number Line and Intervals](#12-数轴与区间--number-line-and-intervals)
    - [典型证明1 | Example Proof 1](#典型证明1--example-proof-1-2)
    - [典型证明2 | Example Proof 2](#典型证明2--example-proof-2-2)
    - [典型证明3 | Example Proof 3](#典型证明3--example-proof-3-2)
  - [1.3 无理数与有理数 | Irrational and Rational Numbers](#13-无理数与有理数--irrational-and-rational-numbers)
    - [典型证明1 | Example Proof 1](#典型证明1--example-proof-1-3)
    - [典型证明2 | Example Proof 2](#典型证明2--example-proof-2-3)
    - [典型证明3 | Example Proof 3](#典型证明3--example-proof-3-3)
...
**命题：若 $a < b < c$，则 $b \in (a, c)$，且 $(a, b) \subset (a, c)$。**

**Proposition: If $a < b < c$, then $b \in (a, c)$ and $(a, b) \subset (a, c)$.**

**证明 | Proof:**

- 由区间定义，$(a, c) = \{x \in \mathbb{R} : a < x < c\}$，$b$ 满足 $a < b < c$，故 $b \in (a, c)$。
- 对任意 $x \in (a, b)$，有 $a < x < b < c$，所以 $a < x < c$，即 $x \in (a, c)$，故 $(a, b) \subset (a, c)$。

---

### 典型证明2 | Example Proof 2

**命题：$[a, b) \cap (b, c] = \emptyset$，$[a, b) \cup (b, c] = [a, c]$ 当 $a < b < c$。**

**Proposition: $[a, b) \cap (b, c] = \emptyset$, $[a, b) \cup (b, c] = [a, c]$ when $a < b < c$.**

**证明 | Proof:**

- $[a, b) = \{x : a \leq x < b\}$，$(b, c] = \{x : b < x \leq c\}$，两区间无公共元素，故交集为空集。
- 并集为 $\{x : a \leq x < b\} \cup \{x : b < x \leq c\} = \{x : a \leq x < b \text{ 或 } b < x \leq c\}$，再加上 $b$ 不属于任一区间，但 $[a, c] = \{x : a \leq x \leq c\}$，所以 $[a, b) \cup (b, c] = [a, c]$（除 $b$ 外所有点都包含，$b$ 作为端点在 $[a, c]$ 中）。

---

### 典型证明3 | Example Proof 3

**命题：区间 $[a, b]$ 可以用不等式 $a \leq x \leq b$ 表示。**

**Proposition: The interval $[a, b]$ can be represented by the inequality $a \leq x \leq b$.**

**证明 | Proof:**

- 由区间定义，$[a, b] = \{x \in \mathbb{R} : a \leq x \leq b\}$，即所有满足 $a \leq x \leq b$ 的实数。
- By definition, $[a, b] = \{x \in \mathbb{R} : a \leq x \leq b\}$, i.e., all real numbers $x$ such that $a \leq x \leq b$.
